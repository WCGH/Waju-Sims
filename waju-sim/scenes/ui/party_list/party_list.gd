...
@onready var member_containers : Dictionary[String, MemberContainer] = {
...
func _ready():
	...
	for key in member_containers.keys():
		member_containers[key].set_role_key(key)
...
func add_debuff(role_key : String, debuff_icon_scene : PackedScene, duration := 0.0, stackable := false, debuff_name := "",  stacks := 1) -> Signal:
	return member_containers[role_key].add_debuff(role_key, debuff_icon_scene, duration, stackable, debuff_name, stacks)
...
func add_debuff_stacks(role_key, debuff_name, added_stacks) -> void:
	member_containers[role_key].add_debuff_stacks(debuff_name, added_stacks)


func remove_debuff_stacks(role_key, debuff_name, removed_stacks) -> void:
