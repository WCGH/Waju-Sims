...
func set_debuff(debuff_icon_scene: PackedScene, new_owner_key: String, debuff_duration := 0.0,
		new_stackable: bool = false, new_stacks := 1) -> void:
	owner_key = new_owner_key
	remaining_duration = debuff_duration
	stackable = new_stackable
	# For permanent debuffs. Don't forget to clear them manually.
	if remaining_duration == 0.0:
		timer.stop()
...
func add_stack(added_stacks := 1) -> void:
	if !stackable:
		return
	stacks += added_stacks
	stacks_label.text = str(stacks)
...
# Removes debuff stacks and if stacks reach zero, removes the debuff.
func remove_stacks(removed_stacks) -> void:
	if !stackable:
		return
	stacks -= removed_stacks
	if stacks < 1:
		self.queue_free()
	stacks_label.text = str(stacks)
