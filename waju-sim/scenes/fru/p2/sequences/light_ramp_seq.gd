...
# Debuff Icon Scenes
const CHAINS_LOCKED = preload("uid://fqlyot7lxk76")
const CHAINS = preload("uid://j32rkij33yfn")
const LIGHTSTEEPED = preload("uid://dfl1fkdqwyuum")
const WEIGHT_OF_LIGHT = preload("uid://5g0r1wriog66")
...
func start_sequence(new_party: Dictionary) -> void:
	...
	strat = FruSavedVariables.save_data["settings"]["p2_lr_strat"]
	if strat is not int or strat >= Strat.size() or strat < 0:
		push_warning("Invalid strat selected. Defaulting to NA.")
		GameEvents.emit_encounter_variable_saved("settings", "p2_lr_strat", 0)
		strat = Strat.NA
...
func four_four_party_setup() -> void:
	...
	if strat == Strat.NA:
		...
		spread_prio = ele_we_spread_prio
	...
	for index in spread_keys_index:
		var lineup_key = lr_north_lineup.find(lightsteeped_keys[index])
		if lineup_key != -1:
			spread_keys.append(lr_north_lineup.pop_at(lineup_key))
		else: 
			lineup_key = lr_south_lineup.find(lightsteeped_keys[index])
			assert(lineup_key != -1, "Error. Failed to find spread key in either N/S lineup.")
			spread_keys.append(lr_south_lineup.pop_at(lineup_key))
		#if index < 4:
