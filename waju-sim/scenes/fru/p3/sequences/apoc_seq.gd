...
func start_sequence(new_party: Dictionary) -> void:
	...
	# Get Strat.
	strat = FruSavedVariables.save_data["settings"]["p3_sa_strat"]
	if strat is not int or strat >= Strat.size() or strat < 0:
		# Fix invalid SavedVariables, defaults to NA.
		GameEvents.emit_encounter_variable_saved("settings", "p3_sa_strat", 0)
		strat = Strat.NAUR
	# Get Apoc Spread.
	apoc_spread = FruSavedVariables.save_data["settings"]["p3_sa_swap"]
	if apoc_spread != Spread.STATIC and apoc_spread != Spread.PERMA:
		# Fix invalid SavedVariables, defaults to Static.
		GameEvents.emit_encounter_variable_saved("settings", "p3_sa_swap", 0)
...
## 41.8
func move_t2_short():
	bait_tank_key = "t2" if !FruGlobal.p3_t1_bait else "t1"
	...
	if (!FruGlobal.p3_t1_bait and t2_swapped) or (FruGlobal.p3_t1_bait and t1_swapped):
		bait_rotation_offset += 180
	# If MUR strat, account for T2 starting on East side.
	if strat == Strat.MUR and !FruGlobal.p3_t1_bait:
		bait_rotation_offset += 180
...
func move_t2_out():
	#var tank_key = "t2" if !FruGlobal.p3_t1_bait else "t1"
...
func party_setup() -> void:
	...
	if FruGlobal.p3_apoc_force_swap:
