...
const DARK_ERUPTION = preload("uid://coaqx2moxiana")
const DARK_FIRE_ICON = preload("uid://o3ixmcdsl53")
...
const SHADOWEYE_ICON = preload("uid://bhgosc57iqmhr")
...
func party_setup() -> void:
	...
	selected_strat = FruSavedVariables.save_data["settings"]["p3_ur_strat"]
	...
	if FruGlobal.p3_selected_debuff != 0:
		...
		if Global.DPS_ROLE_KEYS.has(player_role_key):
			...
			shuffle_list.insert(abs(FruGlobal.p3_selected_debuff - 3), player_role_key)
		else:
	...
	if supp_key:  # Should only be true if random is not selected and player is a support
		...
		shuffle_list.insert(FruGlobal.p3_selected_debuff - 1, supp_key)
...
func on_toggle_bots_visible() -> void:
	var bots_visible = !FruGlobal.p3_ur_hide_bots
	for key in party:
