...
const WYRMCLAW_ICON = preload("uid://ccsxqffct15aq")  # Red
const DARK_BLIZZARD_ICON = preload("uid://drlxknw5uh8cg")
const WYRMFANG_ICON = preload("uid://cbivx02wiiumy")  # Blue
const DARK_ERUPTION_ICON = preload("uid://coaqx2moxiana")
const UNHOLY_DARKNESS_ICON = preload("uid://bpo1f7kh47duk")
const DARK_WATER_ICON = preload("uid://n6aus2x35irp")
const QUIETUS_ICON = preload("uid://d2ounrwibbfkf")
const RETURN_ICON = preload("uid://dp5x6pf45ahah")
const RETURN_ICON_2 = preload("uid://bd2p04c05wc6x")
const STUN_ICON = preload("uid://ct2htn8kif2p6")
const MAGIC_VULN_ICON = preload("uid://pjetpadrwjjn")
const REWIND_MARKER = preload("uid://bbt3gcdnjyro5")
const CLEANSE_PUDDLE = preload("uid://cguyb2vl7cn35")
...
func start_sequence(new_party: Dictionary) -> void:
	...
	if strat is not int or strat >= Strat.size() or strat < 0:
		...
		strat = Strat.NA
	# Apply user settings
	aero_plant = FruSavedVariables.save_data["settings"]["p4_ct_aero_plant"]
...
func move_akh_morn():
	var am_strat = FruSavedVariables.save_data["settings"]["p4_ct_am_strat"]
	if am_strat == 0:
...
func instantiate_party(new_party: Dictionary) -> void:
	...
	if FruGlobal.p4_ct_force_spirit:
...
func na_mur_party_setup() -> void:
	...
	if FruGlobal.p4_ct_selected_debuff != 0:
		...
		shuffle_list.insert(FruGlobal.p4_ct_selected_debuff - 1, player_role_key)
...
func on_toggle_bots_visible() -> void:
	var bots_visible = !FruGlobal.p4_ct_hide_bots
	for key in party:
