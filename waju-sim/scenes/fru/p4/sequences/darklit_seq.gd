...
const CHAINS_LOCKED_ICON = preload("uid://fqlyot7lxk76")
const CHAINS_ICON = preload("uid://j32rkij33yfn")
const LIGHTSTEEPED_ICON = preload("uid://dfl1fkdqwyuum")
...
func start_sequence(new_party: Dictionary) -> void:
	...
	strat = FruSavedVariables.save_data["settings"]["p4_dd_strat"]
	...
	if strat is not int or strat >= Strat.size() or strat < 0:
		...
		GameEvents.emit_encounter_variable_saved("settings", "p4_dd_strat", 0)
...
func move_tank_1_inter_bait():
	...
	dance_strat = FruSavedVariables.save_data["settings"]["p4_dd_dance_strat"]
...
func move_party_am():
	var am_strat = FruSavedVariables.save_data["settings"]["p4_dd_am_strat"]
	if am_strat == 0:
...
func instantiate_party(new_party: Dictionary) -> void:
	...
	if FruGlobal.p4_dd_force_spirit:
...
func na_mana_mur_party_setup() -> void:
	...
	if FruGlobal.p4_dd_force_tether:
